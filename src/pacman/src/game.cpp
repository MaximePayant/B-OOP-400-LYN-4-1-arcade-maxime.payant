        ...
        for (std::size_t col = 0; col < m_map[line].size(); col += 1)
            switch (m_map[line][col]) {
                case 'X':
                    ...
                    break;
                case 'P':
                    module->drawSquare(2, arc::Color::YELLOW, {col * 3 + 0.5 + spacingX, line * 3 + 0.5 + spacingY});
                    break;
                case 'o':
                    module->drawSquare(1, arc::Color::YELLOW, {col * 3 + 1 + spacingX, line * 3 + 1 + spacingY});
                    break;
                case '-':
                    module->drawSquare(1, arc::Color::WHITE, {col * 3 + spacingX, line * 3 + 1 + spacingY});
                    module->drawSquare(1, arc::Color::WHITE, {col * 3 + 1 + spacingX, line * 3 + 1 + spacingY});
                    module->drawSquare(1, arc::Color::WHITE, {col * 3 + 2 + spacingX, line * 3 + 1 + spacingY});
                    break;
            }
    player.draw(module);
    ghost.draw(module);
    module->drawText("PACMAN", 100, arc::YELLOW, {10, 30});
